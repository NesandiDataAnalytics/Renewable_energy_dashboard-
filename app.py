...
@st.cache_data
def load_data():
    df = pd.read_csv("WB_SE4ALL_EG_EGEN_RNEW.csv")
    df = df[['REF_AREA_LABEL', 'COMP_BREAKDOWN_1_LABEL', 'TIME_PERIOD', 'OBS_VALUE']].copy()
    df.columns = ['Country', 'Energy_Source', 'Year', 'Capacity_W_per_capita']
    df['Energy_Source'] = df['Energy_Source'].str.replace('Renewable Source: ', '', regex=False)
    return df
...
<div class="header-box">
    ...
    <p style="margin:10px 0 0 0;opacity:0.85;font-size:1rem;">
        Exploring installed renewable electricity-generating capacity (watts per capita) across 226 countries · 2000–2022 · World Bank SE4ALL
    </p>
</div>
""", unsafe_allow_html=True)

# ── SIDEBAR ───────────────────────────────────────────────────────────────────
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/Color_icon_green.svg/120px-Color_icon_green.svg.png", width=40)
...
if leader is not None:
    ...
    with col_i2:
        st.markdown(f"""<div class="insight-box">
            📉 <b>Lowest capacity:</b> {bottom_country['Country']} with only {bottom_country['Capacity_W_per_capita']:,.1f} W/capita,
            showing a huge gap between nations.
        </div>""", unsafe_allow_html=True)
    with col_i3:
        st.markdown(f"""<div class="insight-box">
            📈 <b>Year-on-year trend:</b> {growth_text if growth_text else "Select a year after 2000 to see growth."}
        </div>""", unsafe_allow_html=True)
...
st.dataframe(
    ...
    column_config={
        ...
        "Capacity_W_per_capita": st.column_config.ProgressColumn(
            "Capacity (W/capita)",
            min_value=0,
            max_value=float(df['Capacity_W_per_capita'].max()),
            format="%.1f"
        )
    }
)
...
st.markdown("---")
st.caption("Data source: World Bank Sustainable Energy For All (SE4ALL) · Indicator: WB_SE4ALL_EG_EGEN_RNEW · Dashboard built with Streamlit & Plotly")